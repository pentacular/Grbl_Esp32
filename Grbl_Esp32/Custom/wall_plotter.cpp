...
*/
void machine_init() {
    grbl_sendf(CLIENT_ALL, "machine_init\r\n");
...
*/
void display_init() {
    grbl_sendf(CLIENT_ALL, "display_init\r\n");
}
...
bool limitsCheckTravel() {
    grbl_sendf(CLIENT_ALL, "limitsCheckTravel\r\n");
    return false;
}
...
bool user_defined_homing(uint8_t cycle_mask) {
    ...
    grbl_sendf(CLIENT_ALL, "user_defined_homing\r\n");
...
/*
  ...
  All linear motions pass through cartesian_to_motors() to be planned as mc_line operations.
...
bool cartesian_to_motors(float* target, plan_line_data_t* pl_data, float* position) {
    // grbl_sendf(CLIENT_ALL, "cartesian_to_motors start\r\n");
    ...
    for (uint32_t segment = 1; segment <= segment_count; segment++) {
        ...
        if (abs(seg_x - cx) > 0.1 || abs(seg_y - cy) > 0.1) {
            ...
            grbl_sendf(CLIENT_ALL,
                       "Kinematic violation: seg_x=%4.2f seg_y=%4.2f seg_left=%4.2f seg_right=%4.2f cx=%4.2f cy=%4.2f\r\n",
                       seg_x,
                       seg_y,
                       seg_left,
                       seg_right,
    ...
    // grbl_sendf(CLIENT_ALL, "cartesian_to_motors end\r\n");
...
bool kinematics_pre_homing(uint8_t cycle_mask) {
    grbl_sendf(CLIENT_ALL, "kinematics_pre_homing\r\n");
...
void kinematics_post_homing() {
    grbl_sendf(CLIENT_ALL, "kinematics_post_homing\r\n");
    // Homing is not implemented.
}
...
void user_tool_change(uint8_t new_tool) {
    // grbl_sendf(CLIENT_ALL, "user_tool_change\r\n");
...
void user_defined_macro(uint8_t index) {
    // grbl_sendf(CLIENT_ALL, "user_defined_macro\r\n");
...
*/
void user_m30() {
    // grbl_sendf(CLIENT_ALL, "user_m30\r\n");
